...
uniform bool periodic_boundary = false;
...
void ComputeDensityPressure(int ix)
{
    ...
    {
        if(1.0-abs(pi.pos.x) < 0.1) w_periodic = 1.5;
        rho += rho_boundary(pi.pos.xyz, wall[3]);
    }
    else
    {
        for(int i=0; i<6; i++)
...
vec3 calc_acc_event(vec3 p)
{
    ...
    acc_event += 470.0*vec3(10.0*p.x, -1.0, 10.0*p.z)*splash*splash_time_pulse;
    
    float t_wave = float(SceneUniforms.Frame-WaveFrame)/60.0;
	float wave_time_pulse = smoothstep(0.0, 0.3, t_wave)-smoothstep(0.4, 0.5, t_wave);
	float wave = smoothstep(0.7, 0.8, p.x);
	acc_event += 300.0*vec3(-0.3, -0.0, 0.0)*wave*wave_time_pulse;
    ...
    acc_event += 1000.0 * vec3(-0.5, 0.0, 0.0) * wake;
...
void ComputeForces(int ix)
{
    ...
    float c_visc = -VISC*10.0*MASS;
    float c_press = MASS;
    float c_cohesion = 0.05;
    ...
    {
        if(1.0-abs(pi.pos.x) < 0.1) w_periodic = 1.5;
        force_boundary(pi.pos.xyz, wall[3], acc_press_i, acc_press, acc_visc);
    ...
    if(periodic_boundary==true)
    {
        ...
        if(pi.pos.x < -1.0+eps) {pi.pos.x += 2.0-2.0*eps;  pi.vel.xyz *= 0.5f;}
        if(pi.pos.x > +1.0-eps) {pi.pos.x -= 2.0-2.0*eps;  pi.vel.xyz *= 0.5f;}
        if(pi.pos.z < -1.0+eps) {pi.pos.z += 2.0-2.0*eps;  pi.vel.xyz *= 0.5f;}
        if(pi.pos.z > +1.0-eps) {pi.pos.z -= 2.0-2.0*eps;  pi.vel.xyz *= 0.5f;}
    }
